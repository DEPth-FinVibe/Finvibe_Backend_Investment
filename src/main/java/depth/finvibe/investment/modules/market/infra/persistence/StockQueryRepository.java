...
import static depth.finvibe.investment.modules.market.domain.QStock.stock;
...
public class StockQueryRepository {
  ...
  public List<Stock> searchByNameOrSymbol(String query) {
    ...
    NumberExpression<Integer> priority = new CaseBuilder()
        ...
        .when(stock.symbol.startsWithIgnoreCase(trimmedQuery)).then(2)
        .when(stock.name.startsWithIgnoreCase(trimmedQuery)).then(3)
        .when(stock.symbol.containsIgnoreCase(trimmedQuery)).then(4)
        .otherwise(5);
